...
{#if ($result_selection.selected)}
    <li class="option">
        <input type="checkbox" id="option-{$result_selection.id}" checked={$result_selection.selected} bind:value={$result_selection.selected} on:click={() => {selectOption($result_selection.id); getRandom(); rotateIcon();}}>
